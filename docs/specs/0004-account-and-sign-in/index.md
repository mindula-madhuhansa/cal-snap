...
- [x] Amend spec 0001's Auth row (it recorded Supabase Auth) and spec 0002's security model, AC-2, and AC-10. Build plan task 23. **Done on 10 August 2026**: spec 0001's Auth and Backend rows, AC-8, and the secret boundary; spec 0002's AC-2, AC-10, all six `user_id` columns, the security model, the deletion path, the compliance note, the configuration note, build step 9, Consequences, and its `rationale.md` and `verify.md`. The amended state was checked against the live database rather than assumed.
