...
>
> **Resolved, 10 August 2026.** The first half of that premise is settled, and it went the way this note warned about: spec [0004](../0004-account-and-sign-in/index.md) chose Clerk, so every `auth.users` foreign key is gone and `user_id` is `text`. Naming it as an explicit constraint rather than assuming it is what made the change cheap: it landed as four edits to one schema declaration and a regeneration, against tables holding zero rows. The amended shape is in [index.md](index.md). The second half, the calorie formula, is still open and still feature 6's.
