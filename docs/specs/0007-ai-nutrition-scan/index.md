...
- [x] `claim_meal_scan` was still executable by `anon`. The migration's `revoke ... from public` does not reach it, because Supabase grants execute on public functions to `anon` directly rather than through `PUBLIC`. It was not a hole (the function raises immediately when the token carries no `sub`, and row level security still applies to every row it touches), but the migration's comment claimed something untrue, which is worse than the grant. Revoked on the live project on 11 August 2026 and confirmed from `pg_proc.proacl`, which now reads `postgres`, `authenticated`, `service_role` and nothing else. The migration file carries the extra `revoke ... from anon` so a fresh project comes up the same way.
