...
- [ ] Build it: `/develop account & sign in`
   - [ ] The door works: the schema change to text identifiers with the new policies, Clerk wired into the splash gate, the combined sign in screen, session routing, and sign out in Settings (AC-1, AC-2, AC-4..AC-8, AC-11, AC-14, AC-16) — all the code has landed and the gate is green (336 tests, lint and typecheck clean). Email sign in reaches the code step on a real device.
     **AC-3 (native Google and Apple) was dropped on 9 August 2026**, on your call. Native Google needs Google Cloud OAuth credentials plus a registered Android signing fingerprint, which is real setup for a way in the emailed code already covers; dropping both rather than only Apple also keeps the App Store's third party sign in rule from applying. Spec 0004 still carries AC-3 and needs an amendment (`/architect account & sign in: drop AC-3`).
     Three things outside the code are still owed: the identity migration has to be run against the live Postgres (the SQL is written and the generated migration in the repo already matches); Clerk has to be registered with Supabase as a third party auth provider with the `role: authenticated` claim added; and Clerk's password attribute has to be made optional, because it is currently required and that blocks sign up in a way AC-1 forbids. Nothing network facing has been exercised yet.
