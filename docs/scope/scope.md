...
| 7 | Snap a meal: AI nutrition scan | Release 1 | done |
...
### 7. Snap a meal: AI nutrition scan · done
...
- [x] Design it (spec): `/architect snap a meal: AI nutrition scan` — one Supabase edge function calling Claude Sonnet 5 with a pinned JSON schema, so a malformed reply is impossible rather than handled. Photos stay on the phone in release 1, the Anthropic key stays in the function, and no schema change is needed at all: spec 0002 designed `meal_scans` for exactly this. A cross check on a second model found two money bugs before any code existed, both now closed: a lost reply could have charged for the same photo twice, and the daily cap was a count followed by an insert, so concurrent requests all passed it.
- [x] Build it: `/develop snap a meal: AI nutrition scan`
   - [x] A photo becomes numbers: the edge function end to end, the narrow scan port with its Supabase adapter, the camera tab and capture screen, image prep at 1024 px, and a plain result list (AC-1, AC-4, AC-9, AC-10, AC-11, AC-15, AC-16, AC-17) — **written in full on 11 August 2026 and green on the gate, deliberately left unticked because nothing is live yet.** The whole phone half is done: the `ScanTransport` port, its Supabase adapter, the third tab, the capture screen, image prep at 1024 px and quality 0.7, the local mirror written clean, and the result list. `supabase/functions/scan-meal/` is **deployed to the live Cal Snap project (version 3, active, `verify_jwt` deliberately off)** and the migration is applied; an unauthenticated POST is refused with a 401 by the function itself. `verify_jwt` had to be turned off: the platform check only accepts tokens Supabase signed, so a Clerk token was rejected at the gateway and the body never ran. The function authenticates itself instead, and reaches the model only after a claim that needs a real `sub`. `ANTHROPIC_API_KEY` is set, and a real plate returned real numbers on a real phone on 11 August 2026: one `low_confidence` row, `claude-sonnet-5`, `v1`, costing `0.527` of a cent
   - [x] One photo cannot cost twice: the atomic insert that is both the cap gate and the pessimistic `failed` row, and idempotency on `scan_id` (AC-7, AC-8, AC-18) — **built and applied to the live database on 11 August 2026**, and read back from the catalogue rather than assumed: `claim_meal_scan` exists, returns `jsonb`, and is `security invoker` (`prosecdef` false), so `meal_scans`'s policy is still the gate on every row it touches. It needed one thing the spec did not name. A conditional insert is one statement but not a *serial* one: under READ COMMITTED two concurrent transactions take their snapshots before either commits, so both read a count under 25 and both spend. A per account `pg_advisory_xact_lock` inside `claim_meal_scan` is what actually serialises them. That also means **this feature adds a hand written migration**, which spec 0007's data model sketch says it would not: no table, column, index or constraint changes and the fingerprint is untouched, but a function is not something the generator can emit. Worth reconciling with `/architect`. **The `anon` grant is closed**: Supabase grants `execute` on public functions to `anon` directly, so the migration's `revoke ... from public` never reached it. Revoked on the live project and confirmed from `pg_proc.proacl`, with the migration file corrected so a fresh project comes up the same way
   - [x] It tells the truth: per item confidence marks and their sentence, the unrecognised screen, the low confidence wording (AC-2, AC-3) — built on 11 August 2026 and green on the gate. Uncertainty is said in words, never left to a hue: an item below `high` carries a written tag, its calorie figure is marked as an estimate, and the sentence above the list counts how many need checking. `unrecognised` draws an empty state with a retake and a library escape and no invented item
   - [x] It fails well: refused permission with its settings link, the offline path, the five failure reasons, the three timeouts and the copy change, a scan surviving a background, retry, and the over cap message (AC-5, AC-6, AC-8b, AC-12, AC-13, AC-19) — built on 11 August 2026 and green on the gate. Every one of the six reasons has its own written sentence, and a test asserts none of them contains the reason code, a provider name, or an HTTP status, so nothing internal can reach a screen. The offline path reuses `network-failure.ts` rather than growing a third copy of that rule. A scan in flight is deliberately **not** cancelled when the screen unmounts: the answer goes into the module level store first and the state second, so returning shows the result rather than an empty camera
   - [x] It holds: Vitest over every pure rule through a fake transport, the accessibility sweep, and a real plate on a real development build (AC-14) — **one of three done.** 64 new tests over the pure rules (the resize decision, the failure sentences, the confidence wording, the display rounding, the cost arithmetic, the local day and cap window), suite now 635 passing across 50 files. They earned their keep immediately: the day window tests **found a real bug and were proven to bite**, because local midnight was being derived from the offset in force at the time of the scan rather than at midnight itself, which put the cap's reset an hour out on both clock change days. The accessibility sweep and the real plate were both done on a real phone on 11 August 2026, and `/test` then added 47 more (the local mirror, the scan adapter, the result store, and the pinned schema), taking the suite to 682 across 54 files
- [x] Verify it: `/check verify snap a meal: AI nutrition scan` — PASS on 11 August 2026. The server half was proven here (the cap gate, idempotency, cross account isolation, and the key absent from a real exported bundle with the search proven to bite), and the six device steps were run by the engineer on a real phone. Two things are owed and neither blocks: the cap under concurrent load, which one session cannot prove, and a defect where a `scan_id` already held by another account raises `23505` instead of claiming, because `meal_scans_pkey` is global rather than per account. Detail in the spec's `verify.md`
- [x] Test it: `/test snap a meal: AI nutrition scan` — 47 new tests on 11 August 2026, suite now 682 passing across 54 files. They cover the four feature 7 files `/develop` left untested: the local mirror (`mirrorScan` writes clean, upserts, and never invents a `raw_response`), the Supabase scan adapter (offline against refusal, and an unrecognised reply treated as unexpected rather than passed to a screen), the module level result store (AC-13), and the pinned prompt and its schema (every object refuses extra properties and requires all of them, which is the whole of AC-17). **One real defect found and pinned**: `countScansOnDay` closes its window at `23:59:59.999` rather than the next midnight, so a scan in the final millisecond of a day is counted on no day at all. Nothing calls it yet, so it costs nobody a scan today; it is held as an `it.fails` test that will start failing the moment somebody fixes the bound
Spec [0007](../specs/0007-ai-nutrition-scan/index.md) · code in `supabase/functions/scan-meal/` (`index.ts`, `prompt.ts`, `cost.ts`, `local-day.ts`), `supabase/migrations/20260811000000_scan_cap.sql`, `src/scan/` (the port, the state machine, the image prep, the sentences, the result view, the capture screen), `src/account/supabase-scan-transport.ts`, `src/data/local/meal-scans.ts`, `src/app/(tabs)/scan.tsx` + `_layout.tsx`, `src/design-system/components/tab-bar.tsx`'s third mark, `app.config.ts`'s three new plugins and their usage descriptions
...
- **Opt in photo sync**: meal photos stay on the phone in release 1, and `profiles.photo_sync_enabled` already exists defaulting to false, with `meals.photo_remote_path` and `photo_synced_at` waiting for it. Turning it on means a storage bucket, its access policy, an orphan cleanup path, and one more thing for account deletion to reach. Needs a decision · from spec 0007
