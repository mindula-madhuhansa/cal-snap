...
- [ ] Build it: `/develop account & sign in`
   - [ ] The door works: the schema change to text identifiers with the new policies, Clerk wired into the splash gate, the combined sign in screen, native Google and Apple, session routing, and sign out in Settings (AC-1..AC-8, AC-11, AC-14, AC-16) — all the code has landed and the gate is green (337 tests, lint and typecheck clean). Two things outside the code are still owed before this can be ticked: the identity migration has to be run against the live Postgres (the SQL is written and the generated migration in the repo already matches), and Clerk has to be registered with Supabase as a third party auth provider with the `role: authenticated` claim added. Nothing network facing has been exercised on a device yet.
